---
name: dbms-digest
description: Auto-run routine for the weekly DBMS digest project — take stock, find missing weeks, generate digests, update index, rebuild RSS, commit and push.
---
...
## Define local skill `weekly-dbms-digest` location

1. Find the directory containing this `SKILL.md`. Let the result be `DBMS_DIGEST_DIR`.
2. Let `WEEKLY_DBMS_DIGEST_DIR` = `${DBMS_DIGEST_DIR}/.claude/skills/weekly-dbms-digest`.
...
3. **Generate the absent digests.** For each missing week, use the **`weekly-dbms-digest`**
   local skill (`${WEEKLY_DBMS_DIGEST_DIR}/SKILL.md`) to build that week's digest over the
...
5. **Keep sources fresh.** The skill may edit its source lists —
   `${WEEKLY_DBMS_DIGEST_DIR}/references/sources.md` (publishers),
   `${WEEKLY_DBMS_DIGEST_DIR}/references/community-sources.md` (forums/chats/channels), and `${WEEKLY_DBMS_DIGEST_DIR}/references/feeds.opml`
